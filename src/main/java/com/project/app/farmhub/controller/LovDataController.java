...
public class LovDataController {
	...
	@PostMapping(value = "/lov-data")
	public ResponseEntity<WebResponse<String>> add(@Valid @RequestBody CreateLovDataRequest request) {
	...
	@PutMapping(value = "/lov-data")
	public ResponseEntity<WebResponse<String>> edit(@Valid @RequestBody UpdateLovDataRequest request) {
		service.edit(request);
		return ResponseEntity.ok(ResponseHelper.ok(PROP_LOV + ErrorMessageConstant.HAS_BEEN_EDITED_SUCCESSFULLY));
	}

	@DeleteMapping(value = "/lov-data/{id}")
	...
	@GetMapping(value = "/lovs-data/courier-service")
